...
- WebSocket 实时推送日志
- 会话连接状态提示
- 待机日志显示（即使脚本未运行也有连接状态信息）
- **定时保存日志** - 可配置间隔自动保存日志到本地文件
- **手动保存日志** - 一键导出当前日志
- **单例会话** - 同名会话只创建一次，避免重复
- **创建重试** - 会话创建失败自动重试，提高成功率
...
- **历史日志** - 按会话分组查看历史日志，支持时间线视图和分组视图切换
- **实时日志** - WebSocket 实时推送日志
- **日志导出** - 一键导出日志文件
- **日志分组** - 按会话智能分组管理
- **定时保存日志** - 可配置间隔自动保存日志到历史记录
- **手动保存日志** - 一键保存当前日志到历史记录
- **日志删除** - 支持删除单个日志文件或整个会话的日志
- **自动重连** - WebSocket 连接断开后自动重连
- **自动滚动** - 日志自动滚动到底部，支持手动暂停和恢复
- **日志行数显示** - 实时显示日志行数和滚动状态
- **会话重试** - 失败或停止的会话支持一键重试
- **固定窗口布局** - 日志区域固定大小，不会无限拉伸

#### 🔧 技术实现细节
日志系统采用 screen 内置日志功能，配合后台广播线程逐行读取日志文件并通过 WebSocket 推送到前端：
- **日志采集**：使用 `screen -X log on` 命令开启会话日志记录，实时将终端输出写入日志文件（UTF-8 编码，支持中文）
- **实时推送**：后台异步线程以 1 秒间隔增量读取日志文件，通过 WebSocket Hub 广播给所有订阅客户端
- **内容清洗**：自动移除 ANSI 转义码、控制字符，确保日志内容干净可读
- **历史日志分类存储**：
  - 容器内 Screen 会话 → `logs/{session_name}/` 目录
  - 宿主机 Screen 会话 → `logs/local_{session_name}/` 目录（local_ 前缀标识）
  - 支持按会话分组查看，时间线视图 / 分组视图双模式切换
- **定时保存**：Dashboard / 日志中心页面可开启定时保存，按设定间隔自动将当前日志归档到历史日志文件夹
- **WebSocket 自动重连**：连接异常断开后，3 秒自动尝试重连，确保日志监控不中断
...
- 自动保存会话日志到日志中心
- 支持中文字符正确显示
- **定时保存日志** - 可配置间隔自动保存日志，日志归档到历史日志 `logs/local_{session_name}/` 目录
- **手动保存日志** - 一键保存当前日志到历史记录
- **历史日志同步** - 宿主机 Screen 日志统一纳入历史日志管理，带 `local_` 前缀分类
- **保存设置持久化** - 支持配置保存间隔和路径

### 📦 脚本库设置
- 查看已安装的 Python 包列表（基于 importlib.metadata 准确检测）
- 添加新的 Python 包（支持版本指定，如 `requests==2.31.0`）
- 支持带 extras 的包（如 `uvicorn[standard]`）
- 安装/移除包
- 一键安装所有配置的包
- 自动同步到 requirements.txt 文件
- 作为自动安装失效时的备用手动管理方式
